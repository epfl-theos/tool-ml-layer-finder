...
class LowDimFinder(object):
    ...
    def _define_groups(self):
        ...
        pairs = get_bonded_pairs(coords, radii_array,
                                 factor=1.+self.params["bond_margin"],
                                 use_scipy=True)
...
def get_bonded_pairs(coords, radii_array, factor=1., use_scipy=True):
    """
    ...
    if use_scipy:
        ...
        pairs = list(tree.query_pairs(2.*max_radius))
        ...
        if len(pairs) > 0:
            ...
            pairs_idx = np.nonzero(
                ((coords1 - coords2)**2).sum(axis=1) - (rad1 + rad2)**2 < 0.)
